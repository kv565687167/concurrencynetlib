...
static void clean(const void* data,unsigned int len,void* arg)
{
...
void buffered_on_read(struct bufferevent *bev, void *arg) {
	...
	while (evbuffer_get_length(bev->input)>0) {
		/* Remove a chunk of data from the input buffer, copying it into our local array (data). */
		nbytes = (evbuffer_get_length(bev->input)> 4096) ? 4096 : evbuffer_get_length(bev->input);
	...
	printf("prepeare\n");
	char* buff=(char*)malloc(50000000);
	printf("begin\n");
	evbuffer_add_reference(bufferevent_get_output(bev),buff,50000000,clean,buff);
	printf("end\n");
}
